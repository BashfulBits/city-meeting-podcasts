...
- **Runner reliability fixes.** Content-addressed S3 uploads now retry transient transfer-manager
  ...
  cleanly; failed claims now log their exception type/message for diagnosis. Beam and Modal deploy
  workflows no longer use protected GitHub environments, so vetted deployments proceed without a
