...
provider labels, add `source.body_any` with explicit alternative labels. For a one-off provider row
whose label is not reliable enough to add permanently, use `source.body_includes` with its exact
provider GUID and expected body label; the audit will flag later rows with that label or newly seen
body labels. List a source's bodies with:
...
- **swagit** — `source.list_url` is a Swagit view page and `source.body` selects one meeting
  ...
  labels to that feed. `source.body_includes` adds exact provider rows for one-off naming
  exceptions. `source.list_urls` (a list) merges several view pages into one feed —
