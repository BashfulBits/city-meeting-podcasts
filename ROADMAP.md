...
| **H4** | ✓ Core shipped; **lifecycle extension in progress** ([GH#970](https://github.com/BashfulBits/city-meeting-podcasts/issues/970), [`review/37`](review/37-stale-feed-lifecycle-and-provider-migration.md)) — replace the permanent stale-feed table with native per-feed review children and committed `active`/`paused`/`dormant`/`retired` decisions; maintainer slash commands create YAML PRs. Stable `source_id` makes provider migrations preserve the append-only archive whether the replacement copies history or starts at cutover. Existing core: catching-up suppressed, stalled → `WARN`; per-provider error-rate tracking and transition-only audit comments. |
