...
| — | **Implemented — retention-cap no-op planning boundary** ([GH#1025](https://github.com/BashfulBits/city-meeting-podcasts/issues/1025)): archive rows enter document/timeline/audio/ASR work only if they survive the exact source-wide merge→prune projection used by final persistence. This first 1.0 efficiency-tranche fix intentionally lands before R6/R7 so those features inherit the no-op boundary; it is a bounded precursor to GH#1013, not a reason to wait for the broader manifest redesign. |
