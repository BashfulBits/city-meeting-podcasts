...
| **5**   | newsletter email delivery · #31 YouTube + other providers · per-feed config via issue comments · structured logging · map browser · #120 audio concat for multi-segment legacy Swagit meetings (single-segment fallback already ships; multi-segment meetings are detected and deferred via materialization backoff until ffmpeg concat lands) |
