...
| **H4** | ✓ Core shipped; **lifecycle extension in progress** ([GH#970](https://github.com/BashfulBits/city-meeting-podcasts/issues/970), [`review/37`](review/37-stale-feed-lifecycle-and-provider-migration.md)) — provider-migration continuity plus the four-state lifecycle foundation shipped in [PR #976](https://github.com/BashfulBits/city-meeting-podcasts/pull/976), and bounded native per-feed stale cohorts shipped in [PR #977](https://github.com/BashfulBits/city-meeting-podcasts/pull/977). Collaborator-authorized slash commands that create deterministic lifecycle YAML review PRs are implemented in the current slice; production conversion of GH#774 remains last. Stable `source_id` preserves the append-only archive whether the replacement copies history or starts at cutover. Existing core: catching-up suppressed, stalled → `WARN`; per-provider error-rate tracking and transition-only audit comments. |
