...
| **R5** | **Implemented locally; shadow-dispatch rollout** — **#4 topic tags / Strong Towns lens**: 37-source-backed flat tags, deterministic episode/chapter annotations, taxonomy-ordered episode rollups, chapter-aware meeting/search payloads, and an Instructor/Pydantic additive LLM path pinned to a single configured model (R13's `LLMRequestPolicy`) are implemented from [`review/14`](review/14-topic-tags-strong-towns-lens.md). LLM candidates are retained with bounded evidence and merged directly alongside rule tags — schema validation and evidence-grounding are the only per-candidate checks; a bespoke calibration matrix and weekly review workflow were built, then removed 2026-07-17 in favor of [`review/27`](review/27-llm-backend-and-provider-routing.md) §6's tournament/champion-routing design, which owns provider/model quality assurance instead. R3 agenda text is episode-level unless it supplies an explicit chapter mapping; transcript timing is the reliable item-level signal. No PR has been opened yet. |
