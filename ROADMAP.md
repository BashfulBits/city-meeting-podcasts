...
| **R11** | **Cross-provider agenda & history network** (new, infra — numbered R11, sequenced here, see note above) — generalizes the existing Legistar/Granicus historical-backfill mechanism (originally "Legistar calendar provider") into three goals: ingest HTML/portal agenda URLs, ingest PDF agenda URLs, and extend meeting history for feeds with limited RSS/API windows. **Maintainer-confirmed implementation decision (2026-07-13):** native Granicus archive pages replace capped RSS as recorded-meeting discovery; verified Legistar calendars become best-effort companion catalogs/backfills for missing video rows and durable agenda-only records. Video rows merge by canonical clip ID; no-video rows persist in a separate append-only calendar catalog and render outside RSS/audio work. Swagit is enriched from its own agenda/minutes links rather than substituted with Granicus. Granicus directly markets three parallel agenda products — **Legistar** (proven), **OneMeeting**, and **Agenda PE** (small/medium-government focused, no confirmed portal pattern yet) — any of which may apply to a Granicus- or Swagit-primary city; adds **CivicClerk cross-referencing** (already a supported provider, now also usable as an auxiliary agenda source for CivicPlus-video cities) alongside these. Feeds R3 (agenda text extraction) and, transitively, R4 (search) and R5 (tags) |
...
| **R4** | **#6** static client-side transcript/meeting search, including metadata-only unavailable recordings and an availability filter — implementation in the current static-search PR |
