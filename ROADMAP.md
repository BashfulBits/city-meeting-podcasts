...
| Pri | Item |
|----:|------|
| **R1** | **Shipped** (#897, 2026-07-13) — per-meeting permalink pages over the append-only archive: playable meetings get player/transcript/chapters/agenda/deep-links; unavailable recordings retain civic metadata + canonical provenance with a clear no-recording notice and no broken player |
| **R10** | **Shipped** (#899, 2026-07-13; follow-up hardening on `main`) — Cloudflare Worker providing the OpenAI-shaped asynchronous enqueue/poll transport for R2's `JobHandle` path, with bounded queue pacing, R2 persistence, and reconciliation-safe polling |
| **R11** | **Core implementation shipped** (#903/#904/#905, 2026-07-13) — Granicus archive-first discovery, calendar composition/backfill, durable no-video calendar rows, and first-party Swagit agenda/minutes links are live. Remaining follow-up is broader vendor coverage (including OneMeeting/Agenda PE/CivicClerk cases) as catalog evidence warrants. Feeds R3 and, transitively, R4/R5 |
| **R2** | **Shipped** (#919, 2026-07-13) — LiteLLM adapter with normalized direct `JobResult` responses and rate-limited `JobHandle` dispatch through R10; provider routing, budget-aware reconciliation, and the H13 LLM verbs are wired. R5/R6 remain its first feature consumers |
| **R12** | **Not started** — L3 design in [`review/28`](review/28-llm-assisted-city-discovery.md); quarterly existing-city discovery and `add-city` bootstrapping remain to implement |
| **R3** | **Shipped** (#920, 2026-07-14) — agenda/portal, bounded backup/packet, minutes, per-item vote, and member-roster extraction are implemented as content-addressed artifacts; no OCR or LLM synthesis in the first pass |
| **R4** | **Shipped** (2026-07-14; commits `2f76744` + `60998a0`) — static client-side meeting/transcript search, metadata-only unavailable recordings, and availability filtering |
| **R5** | **Not started** — topic tags / Strong Towns lens remains L3 design in [`review/14`](review/14-topic-tags-strong-towns-lens.md); implementation awaits the transparent rules, overrides, and later R2-backed assistance |
| **R6** | **Not started** — per-agenda-item cards, auto-summaries, and soundbites remain L3 design in [`review/30`](review/30-cards-summaries-soundbites.md); the extractive/non-LLM path can proceed now that R3 is shipped, while LLM halves depend on R2 |
| **R7** | **Not started** — speaker diarization, minimal minutes-based attendee extraction, and confirmed-speaker pages remain L3 design in [`review/31`](review/31-speaker-diarization-attendee-extraction.md); this is still a 1.0 gate |
| **R8** | **Not started** — front-end design selection, accessibility fixes, and the funding/support surface remain L3 design in [`review/32`](review/32-frontend-design-accessibility-funding.md) |
| **R9** | **Partially shipped; final gate remains** — constraints, SHA/model pinning, Renovate configuration, and dependency-policy checks are in place. Remaining: activate Renovate, add the monthly immutable/checksummed FFmpeg update PR, and optionally harden image installs with `--require-hashes` |
