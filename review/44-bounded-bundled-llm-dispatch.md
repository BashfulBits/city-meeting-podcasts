...
- **Corrected (2026-09-04): checkpointed topic-tag dispatch.** The original topic-tag collector
  deferred its sole `flush()` until both tag passes completed. A 165-minute Actions step timeout
  therefore discarded its entire in-memory collection before a single `enqueue-batch` request,
  despite the configured 2,000 tagger jobs/run and 1,250 pre-labeler jobs/run. The runner now
  enforces the pre-labeler's independent producer cap and flushes accumulated tag jobs before each
  durable record checkpoint. A hard kill can strand only work prepared after the latest checkpoint;
  accepted jobs retain their normal deferred-record recovery. The Worker remains the authority for
  per-day quota admission. No recipe, artifact, pipeline version, or backfill behavior changes.
