...
| Granicus media reliability follow-up | GH#300/#39, [#336](https://github.com/BashfulBits/city-meeting-podcasts/issues/336), [#337](https://github.com/BashfulBits/city-meeting-podcasts/issues/337), [#353](https://github.com/BashfulBits/city-meeting-podcasts/issues/353) | L3 | **In development** · Audio #36/#37 and the isolated GitHub-hosted probe moved the diagnosis beyond simple concurrency/request-shape failure: quiet short fetches, both FFmpeg runtimes, direct `DownloadFile.php`, browser context, and concurrency=2 all passed after a quiet interval, while sustained Audio runs hit immediate archive 403s. The current #337 slices make a planner/source-cache throttle the single persisted materialization attempt (no immediate AudioStage retry), park circuit-deferred work while other providers drain, and perform one half-open canary after cooldown before releasing parked work. Breaker state is now storage-coordinated across shards and tenant-scoped, with a domain emergency only after two distinct Granicus tenant trips; this prevents every shard from replaying the threshold and prevents one tenant from suppressing healthy peers. The isolated manual workflow now offers two modes: a sustained Audio #37 request-count/volume/cooldown matrix, and a low-volume Audio #40 transport discriminator that alternates curl/ffmpeg ordering on one runner and size-caps one curl→local-ffprobe/ffmpeg proof. Keep Granicus caps at 1 local / 2 distributed until ≥3 post-change runs and probe evidence support tuning. A production curl fallback remains gated on curl succeeding where remote ffmpeg fails; #352 duplicate media canonicalization follows validated #337 behavior. Design: [review/12 §Granicus follow-up](12-hardening-and-efficiency.md#granicus-media-reliability-follow-up-gh300--39-follow-up). |
