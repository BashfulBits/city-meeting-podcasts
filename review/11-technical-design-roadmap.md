...
**Status: LIVING · last updated 2026-06-27 (timeline/audio integrity repair series)**
...
| **H18 timeline/audio integrity repair** | [GH#495](https://github.com/BashfulBits/city-meeting-podcasts/issues/495), feed-health timeline findings | L3 | **In development** · distinguish container-duration drift from real stream/decoded audio drift, identify concat segment-duration accumulation, fix source-duration-aware identity detection, and add targeted repair flags that re-plan/re-materialize/re-transcribe only affected records. PR1 adds the cheap sample-clock duration probe and freezes the PR sequence; PR2+ wire read-only audit diagnostics; PR3+ persist repair state; PR4 fixes planner duration basis/GH#495; PR5+ lets normal lanes consume repair flags. No silent full-catalog invalidation; every planner/pipeline bump must state its backfill scope. Design: [review/20](20-timeline-audio-integrity-repair.md). |
