...
| Granicus media reliability follow-up | GH#300/#39, [#336](https://github.com/BashfulBits/city-meeting-podcasts/issues/336), [#337](https://github.com/BashfulBits/city-meeting-podcasts/issues/337), [#353](https://github.com/BashfulBits/city-meeting-podcasts/issues/353) | L3 | **In development** · Audio #36/#37 and the isolated GitHub-hosted probe moved the diagnosis beyond simple concurrency/request-shape failure: quiet short fetches, both FFmpeg runtimes, direct `DownloadFile.php`, browser context, and concurrency=2 all passed after a quiet interval, while sustained Audio runs hit immediate archive 403s. The current #337 slice makes a planner/source-cache throttle the single persisted materialization attempt (no immediate AudioStage retry), parks circuit-deferred work while other providers drain, and performs one half-open canary after cooldown before releasing parked work. A manual isolated sustained probe now measures exact Audio #37 Fort Worth objects, serial request count, progressive transfer duration, cooldown recovery, and concurrency last. Keep Granicus caps at 1 local / 2 distributed until ≥3 post-change runs and sustained-probe evidence support tuning. Request-shape fallback remains gated; #352 duplicate media canonicalization follows validated #337 behavior. Design: [review/12 §Granicus follow-up](12-hardening-and-efficiency.md#granicus-media-reliability-follow-up-gh300--39-follow-up). |
