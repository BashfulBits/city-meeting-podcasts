...
Swagit terminal-media, parser-drift, and run-history telemetry remain authoritative inputs. First reader:
`/admin/status`; later readers: Phase-R pages/query API. Keep the schema storage-adapter-neutral so
review/17's SQL path can index it later. Promote only when a Phase-R surface needs the finer detail.
