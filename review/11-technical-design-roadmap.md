...
| H6b separate audio + ASR workflows, sharded | #1, R1 | L3 | committed · after H11b (H11b shipped the interim combined `enrich.yml`); split into `audio.yml` + `asr.yml` (own concurrency groups, #269), sharded by `source_key` + **scoped `push_state`** (the `only_prefixes=`/`full_run=` hooks landed in H11b — no cross-shard clobber); align-only vs transcribe-only lanes via the H13 `local` adapter; leases reserved for H14 |
