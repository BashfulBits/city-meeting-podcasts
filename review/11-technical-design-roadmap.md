...
**Status: LIVING · last updated 2026-07-24 (GH#1025 prospective-retention planning boundary
implemented as a pre-R6/R7 efficiency prerequisite; see CHANGELOG.md for the corresponding entry)**
...
| Retention-cap no-op planning boundary | [GH#1025](https://github.com/BashfulBits/city-meeting-podcasts/issues/1025) | L3 | **Implemented in the first 1.0 efficiency tranche, before R6/R7 rollout.** Archive-first provider rows now enter document/timeline/audio/ASR planning only if they survive the exact shared source-wide merge→prune projection that final persistence uses. Full fresh observations still reach the authoritative append/prune write; retained rows with a missing artifact pointer remain eligible for ordinary content-addressed credit; repair flags cannot bypass retention. This is the bounded precursor to GH#1013's broader unchanged-episode dirty scheduling and deliberately does not wait for that manifest redesign. No retention-pin or physical-media identity rule was introduced, and no pipeline version or artifact backfill is required. |
| Swagit list-page Worker fallback | [PR #1011](https://github.com/BashfulBits/city-meeting-podcasts/pull/1011) / [PR #1026](https://github.com/BashfulBits/city-meeting-podcasts/pull/1026) | L3 | **Shipped** · same shared-GitHub-egress-reputation signature as H16 Granicus, on a different host class. Paired local/GitHub-Actions probes (PR #1011: `scripts/probe_swagit_transport.py`, `.github/workflows/swagit-probe.yml`) plus production Audio #257-#259 and the LLM tag-lane enrich showed every known Swagit tenant's list/view page (`SwagitProvider.fetch_episodes`) returning `403` (`server: awselb/2.0`, an AWS load balancer) from GitHub Actions egress while the same requests succeeded cleanly from a residential network under heavier load (a full pagination burst + cross-tenant concurrency, zero degradation) — ruling out our own request pattern/concurrency as the cause. PR #1026 ships `workers/swagit-list-proxy`, a sibling Cloudflare Worker to `granicus-media-proxy` narrowly scoped to `/views/...` list pages (bearer auth, tenant-hostname allowlist, one bounded `page` query param, no redirects), and `citypods/swagit_proxy.py`'s `get_with_worker_fallback` wraps the list-page GET with the same direct-first, single-Worker-attempt-on-403 shape production already uses for Granicus (unset `SWAGIT_PROXY_*` is a no-op). Deployed and confirmed end-to-end (a direct authenticated request from a residential network returned `200`). Not yet covered, both natural follow-ups if needed: per-tenant transport telemetry (`SwagitProvider` methods don't take `ctx`, unlike Granicus's ffmpeg call sites — a broader interface change) and `fetch_chapters`/video-page fetches (not observed failing; the Worker doesn't accept that path shape yet). |
...
shipped in commits `2f76744` and `60998a0`. R6, R7, and R8 remain design-ready but unimplemented;
**R5 is implemented locally** (topic tags + calibration, `review/14`, PR #945, not yet merged as of
this paragraph's update); R12 is implemented on an unmerged PR. R9 is partially shipped: the policy, pins, Renovate configuration, and checks exist;
...
| **LLM quality tournament & champion routing** (new, Infra) | new | **L3, not yet built, issues not yet cut** · breakout of `review/27` §6, 2026-07-17 | [`review/34`](34-llm-quality-tournament-champion-routing.md) · weekly, per-verb pairwise tournament across the three providers (round-robin, judge never grades its own family), bias-mitigated (order-swap, tie policy, human-calibration check); a challenger clearing a cost-gated win-rate margin is proposed via a checkbox GitHub issue, never applied automatically. Decides *which provider* is champion for a verb — complementary to, not a substitute for, review/35's per-candidate matrix |
...
| Topic tags / Strong Towns lens | #4 | **Implemented locally** (2026-07-16; [PR #945](https://github.com/BashfulBits/city-meeting-podcasts/pull/945)) | [`review/14`](14-topic-tags-strong-towns-lens.md) · 37-source-backed flat tags; deterministic episode/chapter annotations with taxonomy-ordered episode projection; explicit R3 chapter mappings only, transcript timing otherwise; chapter-aware R4/R1 payloads; Instructor/Pydantic additive suggestions through Gemini's direct transport; candidates retain bounded transcript/document evidence; generic sparse calibration matrix and weekly review automatically control visibility, starting with a 100% feature/provider-model fallback; future crowd-sourced edits remain moderated proposals |
