...
| #39 per-provider rate limiting (incl. Retry-After clamp) | #39 | L2→L3 | **Shipped** ([#274](https://github.com/BashfulBits/city-meeting-podcasts/issues/274)) · process-global `HostRateLimiter` (per-registrable-domain cap from `provider_rate_limits`) acquired by **both** `GuardedHTTPAdapter.send` **and** the ffmpeg/ffprobe fetch paths (`media.py`, `concat.py`) — the H6b storm was ffmpeg, not `requests`, so capping only the adapter would have missed it; Granicus follow-up adds B2-compatible soft `provider_distributed_leases` around media reads across the four audio shards (initially 6 after 2026-06-15 overlap probes, reduced to 2 by GH#300 Phase 1) plus `rate_limited` classification and a run-local circuit breaker; Granicus 403 backoff lifted into the shared retry (403 in `status_forcelist`, Retry-After clamp kept); also fixed the H6b regressions it surfaced: truncation guard (encode < 50 % of declared duration → #120 backoff, not hosted), source-atomic weighted `shard_assignment` (no empty `audio (0)`, large sources packed first), responsive 0.5 s ffmpeg poll (honest `seconds=`). Design: [review/12 §#39](12-hardening-and-efficiency.md#39--per-provider-rate-limiting-sequence-with-h6b) |
| Granicus media reliability follow-up | GH#300/#39, [#336](https://github.com/BashfulBits/city-meeting-podcasts/issues/336), [#342](https://github.com/BashfulBits/city-meeting-podcasts/issues/342), [#343](https://github.com/BashfulBits/city-meeting-podcasts/issues/343) | L3 | **Committed** · three-phase sequence: (1) test distributed/process-local concurrency caps (2,3,4,5 slots vs 2/1 rate-limits) to reduce 403 prevalence; requirement: zero stuck files, test period confirms drain vs throughput. Issue #336 implements the correctness prerequisite exposed by Audio #30: renewable waiting/held leases with stable FIFO election, stale-owner diagnostics/cleanup, post-slot circuit admission, atomic trip logging, and per-domain lease/breaker telemetry. **Shipped (#342):** Audio #32/#33 showed one shard occupying both distributed Granicus slots even with a local cap of one — the ffmpeg/ffprobe guard joined the distributed lease election before the process-local `HostRateLimiter` slot, so several of one process's own threads could win every distributed candidate while they queued locally. The guard now acquires the local slot first (both ffmpeg paths and ffprobe), restoring cross-shard fairness; post-slot circuit admission and aggregate slot limits are unchanged. **Shipped (#343):** Audio #33 shard 3 still produced groups of four direct 403s against a threshold of three — the circuit was only recorded/opened by the higher-level materialization caller, after the subprocess boundary had already released both provider slots, leaving a window for a queued waiter to acquire the freed lease and start one extra ffmpeg process per threshold crossing. `_raise_if_rate_limited` now records the failure and atomically opens the circuit from inside the same `with` block that holds the slots (both `subprocess.run` and monitored/`Popen` paths), and the caller skips re-recording when the boundary already did. (2) pause Granicus media-fetch in `contracts.yml` when `audio.yml` active, auto-retry on contract failure; (3) request-shape alternatives only if phases 1–2 don't resolve (add headers, direct DownloadFile.php, HLS discovery). Escalate stuck downloads (8+ attempt failures) as high-priority GH issues. Design: [review/12 §Granicus follow-up](12-hardening-and-efficiency.md#granicus-media-reliability-follow-up-gh300--39-follow-up). |
