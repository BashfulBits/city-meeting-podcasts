...
that R2's `Backend` and R5's local tagging/calibration implementation are available, so every LLM-assisted
path across all three Parts should reuse R5's generic evaluator and remain shadow-only until calibrated;
the non-LLM paths don't depend on that gate and can ship independently. **Cards:** extractive first
