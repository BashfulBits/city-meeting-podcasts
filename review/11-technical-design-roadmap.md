...
**Status: LIVING · last updated 2026-07-02 (withheld/dead episode lifecycle, GH#795)**
...
| **H14d GPU worker memory/admission optimization** | new | L3 | **Committed** · after H14b/H14c land their initial pull workers and CAS-backed RSS/GPU-VRAM telemetry, turn those measurements into conservative GPU admission and chunking policy before H9's final GPU-type profiling. Default remains one active ASR/diarization job per GPU container; `max_claims` is sequential per invocation, not per-container concurrency. H14d designs measured `(backend, gpu_type, model, compute_type, duration_bucket, task)` headroom, fixed-then-VAD chunk evaluation, optional chunk checkpoints for preemption/OOM retry waste, and explicit ASR recipe/version handling before any chunking change alters transcript output. Design: [review/12 §H14d](12-hardening-and-efficiency.md#h14d--gpu-worker-memoryadmission-optimization). |
...
| Runtime/dependency maintenance automation | new | L1→L2 | **Final Phase-R release item; completing Phase R is the 1.0 gate. Policy + foundation in progress** — normative contract now in [`review/22`](22-dependency-and-reproducibility-policy.md) (executes this item): Renovate for Python/Docker/Actions/npm (chosen over Dependabot for regex-bumping ffmpeg/model pins); compiled hash-pinned Python `constraints/*.txt`; all third-party Actions SHA-pinned (GH#734); HF model revisions pinned (GH#498); a monthly immutable-URL/checksum FFmpeg update PR gated by a **per-source** image smoke test; a light-touch Dashboard-approval flow for output-affecting bumps; CI guards keep pins from rotting. Weekly image builds verify pinned inputs but do not silently advance them. Absorbs the remaining useful scope of closed GH#339. |
...
| Work distribution & sharding for distributed ASR workers | new (Infra) | L2 · active Stage 1/Stage 2 substrate promoted to **H17**; [`review/18`](18-work-distribution-sharding.md). Later scaling refinements remain trigger-gated |
