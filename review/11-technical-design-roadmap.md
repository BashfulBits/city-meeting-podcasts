...
**Status: LIVING · last updated 2026-06-20 (routing-aware ASR shard cost + H14 planner contract)**
...
| Granicus media reliability follow-up | GH#300/#39, [#336](https://github.com/BashfulBits/city-meeting-podcasts/issues/336), [#337](https://github.com/BashfulBits/city-meeting-podcasts/issues/337), [#353](https://github.com/BashfulBits/city-meeting-podcasts/issues/353) | L3 | **In development** · Audio #36/#37 and the isolated GitHub-hosted probe moved the diagnosis beyond simple concurrency/request-shape failure: quiet short fetches, both FFmpeg runtimes, direct `DownloadFile.php`, browser context, and concurrency=2 all passed after a quiet interval, while sustained Audio runs hit immediate archive 403s. The #337 slices make a planner/source-cache throttle the single persisted materialization attempt, park circuit-deferred work while other providers drain, and perform one shared half-open canary after cooldown. Breaker state is storage-coordinated, tenant-scoped, and escalates domain-wide only after two tenant trips. The 2026-06-20 paired transport artifact returned 403 for all 12 GitHub direct cases while the same objects succeeded from a Mac. The first Worker artifact then produced ffmpeg audio for all four same-runner objects; Fort Worth honored Range and its full 89.9 MB object validated locally, while Arlington/Pflugerville authenticated but ignored Range. A direct-first, one-Worker-attempt production fallback is implemented under the unchanged 1-local / 2-distributed envelope, with per-tenant Worker-fallback telemetry (`worker_fallback_attempts`/`successes`/`failures`) added to the circuit so usage is measurable from the build log and run summary, a fail-safe that disables the fallback on misconfigured proxy secrets instead of aborting the shard, and the echoed Worker endpoint scrubbed from logs. One full production-recipe Arlington/Pflugerville encode is required before merge; the three post-activation Audio runs accept it when those counters show successes ≈ attempts / failures ≈ 0 per tenant, Granicus circuit trips/deferrals fall to ~0, and episode identity is unchanged — after which a sticky-Worker routing change and Granicus circuit-machinery simplification become data-driven options (config-only rollback is unsetting the two secrets). #352 duplicate media canonicalization follows validated recovery behavior. Design: [review/12 §Granicus follow-up](12-hardening-and-efficiency.md#granicus-media-reliability-follow-up-gh300--39-follow-up). |
...
→ local-ASR duration admission guard (narrow H14 prerequisite; implemented, unreleased)
→ H14 Modal + Beam free-tier transcription adapters (async dispatch from `asr.yml`; long-audio-safe;
H5 leases go live) → H9 combined-throughput/routing evaluation. The execution-backend **interface
design** (§5.5/H13) is a
