...
| **Agenda, packet, minutes text, votes, and roster extraction** (new, Infra) | new | **L3** · **ROADMAP R3, expanded 2026-07-14** | [`review/29`](29-agenda-text-extraction.md) · [`review/30`](30-minutes-text-votes-rosters.md) · agenda/portal text plus bounded per-item backup-link union are content-addressed sidecars; minutes links discovered in agendas fill only missing `links["minutes"]` and never override provider-canonical links; `MinutesTextStage` extracts whichever effective minutes link exists; deterministic per-item vote and member-roster artifacts preserve evidence/source URLs and feed roster candidates to the later diarization lane. No OCR or LLM is required for the first pass; a small-model fallback remains optional and non-canonical. |
| Static transcript search | #6 | **Implemented in [PR #924](https://github.com/BashfulBits/city-meeting-podcasts/pull/924) (pending merge)** · deterministic, cacheable per-source MiniSearch shards; every available transcript indexed with disclosed city/body coverage; agenda/backup/minutes/vote/roster fields; metadata-only records; and progressive city/body/topic/date/availability filtering | [`review/13`](13-per-meeting-pages-and-search.md) Part B |
