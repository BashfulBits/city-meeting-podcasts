...
**Status: LIVING · last updated 2026-06-20 (canonical ASR shard planner + restore fan-out removal)**
...
| Work distribution & sharding for distributed ASR workers | new (Infra) | L2 · [`review/18`](18-work-distribution-sharding.md). **Stage 1** (per-`(source,uid)` transcribe planning + ownership-keyed per-episode merge) **L2→L3 do-next** — spreads a hot source across shards + installs the write path external workers reuse; audio stays source-atomic. **Stage 2** (pull-based CAS lease ledger on R2; workers claim episodes) L1→L2, trigger-gated by the first external worker — build H14b/H14c against this claim contract to avoid GPU-worker rework. Builds on [`review/17`](17-state-store-backend-evaluation.md) R2/CAS; resolves its `episodes.json` swing case toward R2-CAS |
