...
| **Speaker diarization + minimal attendee extraction + per-speaker pages** | #7, #14 | **Implemented locally; rollout gated** (2026-08-22) · **ROADMAP R7** | [`review/31`](31-speaker-diarization-attendee-extraction.md) · native pyannote diarization now shares the versioned `speakers_*` artifact family with provider output (provider source takes precedence), preserves transcript text, and projects qualified identity assignments into PR #1271 pull quotes. The private registry derives attendance vocabulary from minutes/votes, requires two approved reference meetings for voice profiles, and only permits provisional recurring-official names after 30 days, 30 reviewed cases, and ≥95% precision for the body/capture context. Official minutes silently reconcile a provisional match against that roster or remove it. Public speaker pages contain only attributed quotes and source meeting/time links; embeddings, clips, scores, and review history remain private. Timed transcript recognition and title-led announcements now feed a human-approved golden-reference sub-issue queue; they never assign names directly. The Denton shadow pilot is enabled, while public automatic naming remains gated on the pyannote-vs-WeSpeaker benchmark and calibration labels. |
