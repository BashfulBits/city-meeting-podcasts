...
| **System architecture evolution & refactoring** (umbrella) | new | **L2 program design** (reviewed 2026-09-04) | [`review/45`](45-system-architecture-evolution-and-refactoring.md) · Gated architectural program, not an implementation-ready omnibus. Each selected slice needs its own L3 breakout/issue with a measured trigger, file/function plan, data and ownership contract, migration/rollback, backfill statement, and acceptance evidence. The state-store candidate now requires generation/pointer semantics and a field-owner matrix; v1 retirement requires a producer/R2/deployment drain proof; SSRF pinning is a security research gate; module extraction preserves current imports via non-conflicting implementation namespaces. Review/26 duration normalization and review/34 tournament work are explicitly superseding/owning designs. |
