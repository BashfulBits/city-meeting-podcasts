...
| Topic tags / Strong Towns lens | #4 | **Implemented locally; shadow-dispatch rollout** (2026-07-16; calibration matrix/weekly-review workflow removed 2026-07-17 in favor of `review/27` §6; no PR yet) | [`review/14`](14-topic-tags-strong-towns-lens.md) · 37-source-backed flat tags; deterministic episode/chapter annotations with taxonomy-ordered episode projection; explicit R3 chapter mappings only, transcript timing otherwise; chapter-aware R4/R1 payloads; Instructor/Pydantic additive suggestions through a single pinned model (R13's `LLMRequestPolicy`); candidates retain bounded transcript/document evidence and are merged directly, with no per-candidate confidence gate — provider/model quality assurance is [`review/27`](27-llm-backend-and-provider-routing.md) §6's future tournament/champion-routing design; future crowd-sourced edits remain moderated proposals |
...
that R2's `Backend` and R5's local tagging implementation are available, so every LLM-assisted path
across all three Parts is schema-validated and evidence-grounded, then exposed directly, the same
pattern R5 ships (`review/14`) — reusing R5's now-removed per-candidate calibration policy is no longer
the plan; provider/model quality assurance for all three Parts is `review/27`'s future tournament/
champion-routing design (2026-07-17, see `review/14`'s dated section and `review/30` §0);
the non-LLM paths don't depend on that and can ship independently. **Cards:** extractive first
