...
| **H4 stale-feed lifecycle + provider-migration continuity** | umbrella [GH#970](https://github.com/BashfulBits/city-meeting-podcasts/issues/970), slices [#971–#975](https://github.com/BashfulBits/city-meeting-podcasts/issues/971) | **L3, development started 2026-07-21** | [`review/37`](37-stale-feed-lifecycle-and-provider-migration.md) · provider-migration continuity and the four-state lifecycle foundation (#971/#972) shipped in [PR #976](https://github.com/BashfulBits/city-meeting-podcasts/pull/976); #973's capped native cohort/incident reconciliation shipped in [PR #977](https://github.com/BashfulBits/city-meeting-podcasts/pull/977). Durable human decisions are only `active` / finite `paused` / polling-but-stale-suppressed `dormant` / archive-preserving non-polling `retired`. #974's collaborator-authorized `/stale pause\|dormant\|retire` commands create or update deterministic validated review PRs for the marker-linked feed YAML in the current change; #975 converts GH#774 after that command path lands. A stable optional `source_id` preserves the append-only record namespace across provider/source changes; the migration dry-run supports both copied-history and forward-only replacements, preserves provider-independent UIDs/artifacts, and fails closed on ambiguous joins with an explicit override escape hatch. No pipeline-version bump or automatic audio/ASR backfill. |
