...
| **Provider catalog reconciliation** | new | **Slice 1 shipped (observe and propose); PR C + Slices 2–4 L3** | [`review/48`](48-provider-catalog-reconciliation.md) · weekly catalog/health/candidate probes under the v2 dispatch pause (#1848), providers as plugins, one rolling decision issue. Next: PR C per-route structured-output shaping in the v2 Worker (producers send only the schema; four verified methods; empty structured replies fail loudly); Slice 2 `/apply` → curated additions PR; Slice 3 automatic removals + lane repair + `route_retired` job rescue; Slice 4 bounded limit maintenance |
