...
The fix series (GH#702). The served=probe inversion was split into its own PR after PR2 because the
...
- **PR1 (#703, merged):** consolidate the EDL/cue clock into one `timeline.edl_duration` primitive that
...
- **PR2:** `SilencePlanner` measures the source's stream-sample duration (`_probe_stream_sample_duration`,
  mirroring `concat.py`) and uses it for trailing-silence detection and the final keep-span, recording
  `duration_basis="stream-sample"`. Root-cause fix; re-planned episodes get a corrected EDL the renderer
  matches.
- **PR3:** make the probed hosted-stream duration authoritative for `audio_duration_served` (remove the
  `_backfill_served_duration` / `_refresh_served_duration_from_audio` EDL-clobber), repoint/retire the
...
- **PR4:** decouple `_build_streaming_single_source_filter` from `PODCAST_SPEECH_PROFILE` and guard
...
- **PR5:** cohort remediation for all feed-health-detected mismatches + audit threshold de-noise.
- **PR6 (gated, build-but-do-not-merge):** `silence:3` catalog version bump for the permanent guarantee.
