...
| Area | Modules |
|---|---|
| **Providers** | `providers/{base,granicus,legistar,civicplus,civicclerk,swagit}.py` — `MeetingProvider` Protocol + registry; each normalizes to the episode model. Granicus is archive-first; verified calendar companions are explicit config, never guessed at runtime. Swagit follows paginated archive views, keeps first-party video discovery and carries its per-video agenda/minutes links directly from the archive list page. City-wide aggregate views reconcile by provider GUID against dedicated feeds before materialization/rendering. |
