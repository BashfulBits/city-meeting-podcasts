...
The pipeline routes LLM jobs across 10 independent providers via [`config/provider_limits.yml`](config/provider_limits.yml) (compiled to both `workers/llm-dispatch-proxy/src/dispatch_limits.json` and the Python `citypods/compute/llm_routes.json`). The generated catalog contains 55 physical provider/account routes representing 36 deduplicated logical models; every route supports direct LiteLLM and asynchronous dispatch. A route may declare a `model_key` when provider-qualified selectors are aliases for the same model family. The compiler emits `model_aliases`, normalizes the compiled route's logical `model`, and leaves each physical `route_id` independent for quota and credential accounting. DeepSeek V4 Flash 0731 is one pool across DeepSeek, SiliconFlow, and OpenCode; Nemotron 3 Ultra free is one pool across OpenRouter, Kilo, and OpenCode. The pool starts at the 22B Codestral structured-output specialist and otherwise favors 24B+ or frontier-capacity models appropriate to the documented task tier.
