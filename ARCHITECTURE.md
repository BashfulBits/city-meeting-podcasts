...
| **LLM dispatch recovery** | V2 enqueue/poll batches preserve per-job outcomes; only unknown outcomes enter one bounded recovery round (large sets remain batched, small sets are isolated), and payload-free counters show batch, retry, singleton, and schema-correction traffic. |
