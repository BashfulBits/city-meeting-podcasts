...
| **LLM deferred index** | `compute/llm_deferred.py` maintains an advisory B2 pointer index at `state/llm_deferred_index/pending/<model>/<recipe_hash>.json`, using the existing `ROUTES` keys (no time-bucket layer — nothing in this codebase persists a genuine future retry time for a deferred record today, so every pointer is always "due now"; see the module for how to extend this if that changes). The sweep checks `LLMBudget.available()` before listing route partitions, then re-reads canonical records before acting. `scripts/llm_deferred_sweep.py --repair-index` rebuilds pointers from the canonical listing and completes migration only after every canonical read succeeds; until then sweeps retain the full-list dual-read fallback. Exhausted transient object reads raise typed `StorageReadUnavailable` errors: best-effort snapshot/sweep paths report the affected key and continue with independent records, while strict reads propagate auth/configuration failures. Index repair does not compact pointers or mark migration complete when a canonical record was unavailable, preventing a transient read outage from stranding work. This is best-effort B2 state, not R2 CAS coordination, and does not change LLM outputs or retry semantics. |
