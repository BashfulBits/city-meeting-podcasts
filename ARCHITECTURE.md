...
| **Enrichment stages** | `stages.py` — `EnrichmentStage` Protocol + `default_stages()` (`Chapters→Timeline→Remap→Audio→Transcript→Links→AgendaText→MinutesText→Tags`); document stages retain content-addressed agenda/minutes text, backup-link manifests, per-item vote evidence, and member-roster candidates without changing audio bytes. `TagsStage` uses transcript timing for chapter windows and only consumes explicit R3 agenda-item mappings, never guessed document order. Its chapter-only LLM path and deterministic matcher both write one unified `Episode.llm_tag_candidates` ledger: rule candidates retain authored phrase, exact match, and rule-version provenance, LLM candidates retain model/evidence provenance, and projection separates existence, 12/90% tagger admission, 50/95% pre-labeler overlay qualification, and display. A qualified likely-incorrect evaluator result suppresses display without deleting the candidate. Deterministic include/exclude observations are retained as compact tagging telemetry for the weekly taxonomy audit. The production tagger receives rich chapter context; the reduced pre-labeler context is evidence-centered, token-aware, greedily batched to the route ceiling, and records input estimates, truncation policy, and input digest. Superseded/legacy candidates remain hidden historical ledger rows. The existing in-memory triage/fingerprint, serialized state loading, and wall-clock defer behavior remain in place; policy-only changes re-project stored candidates without a blanket LLM recall, while chapter/content/recipe changes create ordinary gradual work. Render-only builds do not construct an LLM backend or require LLM secrets. |
...
| **`deepseek/deepseek-v4-pro`** (NVIDIA build leg) | 💰 **Tier 5 (Frontier Paid)**, free here | NVIDIA build (free) + DeepSeek Direct (paid, $0.435-0.66/M) | 1,000k tokens | Overflow target for Mistral Medium quota exhaustion | Same model as the paid DeepSeek Direct route, free via NVIDIA's hosted catalog |
