...
Each feed projects the shared source archive through its optional `source.body` substring filter.
Provider-specific naming drift can additionally be declared with `source.body_aliases`; aliases
match only an exact normalized provider label (or the duplicate label emitted by some archive HTML),
so a phrase mentioned inside another body's title does not become a false positive. Body filters
and aliases are excluded from `source_key()`, keeping combined and per-body feeds on one record and
audio store.
