...
| **Transcripts** | `asr.py` — forced alignment (stable-ts) / fresh transcription (faster-whisper) with align-error fallback; emits a clean **segment-cue VTT** (served via `<podcast:transcript>`) **plus a word-level JSON sidecar** (`…-asr-<recipe>.words.json`) for search/clips/diarization. `stages.py` discovers provider transcript endpoints, including Swagit's conventional `/videos/{id}/transcript` fallback when a video page omits the link, stores source bytes under content-addressed `provider-` keys, and persists bounded probe backoff so known misses and already-stored documents are not fetched every run. It applies an explicit provenance policy: provider VTT with inline word timing may be served as `provider-native`; cue-only VTT/SRT/TXT is wording input for stable-ts and is served as `provider-aligned`; no provider document uses fresh ASR (`asr`). Active records persist text/timing/selection provenance, while H15 source/body routes can switch the served artifact dynamically. ASR keys are content-addressed by transcript media/timeline + ASR recipe, provider-align keys by provider text + align recipe, and provider-native keys preserve the provider object; missing word sidecars are never considered done. Provider source records retain candidate/known-good/history for rollback and expose the original document separately. Selected computed provider-aligned VTTs can produce a separate content-addressed `speakers.json` block when they already contain `SPEAKER: text` labels, and diarization failures never discard the active transcript. `bench.py` — `asr-bench` diagnostic. |
...
- **Rate-limited LLM dispatch** → `workers/llm-dispatch-proxy` is a separate Cloudflare Worker and
  ...
  ledger plus provider-wide pacing** (`state/dispatch_budget.json`, R2, mirroring `llm_budget.py`'s versioned
  continuous per-route request schedule, provider request schedule, day window, cost, `blocked_until`,
  and `inflight` shape),
...
The pipeline routes LLM jobs across 10 independent providers via [`config/provider_limits.yml`](config/provider_limits.yml) (compiled to both `workers/llm-dispatch-proxy/src/dispatch_limits.json` and the Python `citypods/compute/llm_routes.json`). The generated catalog contains 52 physical provider/account routes representing 36 deduplicated logical models; every route supports direct LiteLLM and asynchronous dispatch. A route may declare a `model_key` when provider-qualified selectors are aliases for the same model family. The compiler coalesces selector-only duplicates that share a provider/account/upstream tuple, emits `model_aliases`, normalizes the compiled route's logical `model`, and leaves each physical `route_id` independent for quota and credential accounting. DeepSeek V4 Flash 0731 is one pool across DeepSeek, SiliconFlow, and OpenCode; Nemotron 3 Ultra free is one pool across OpenRouter, Kilo, and OpenCode. The pool starts at the 22B Codestral structured-output specialist and otherwise favors 24B+ or frontier-capacity models appropriate to the documented task tier.
...
| **`nvidia/nemotron-3-ultra-550b-a55b:free`** | 🏆 **Tier 1 (Frontier Open Free)**<br>550B MoE (55B active) | OpenRouter + Kilo + OpenCode | 128k tokens | 10 RPM / 20 RPM / 20 RPM<br>200 Free RPD each | Available in pool | Elite reasoning verification and complex cross-examination validation |
