...
| **LLM scheduling** | `compute/llm.py`, `compute/llm_policy.py`, `compute/llm_budget.py`, and `compute/llm_scheduler.py` — the LiteLLM adapter plus provider-neutral route capabilities, transport-constrained selection, and CAS-backed `state/llm_budget.json` quota/cost reservations. Policy-bearing calls settle provider usage after any attempted call and release only when no provider call occurred; callers without policy retain the pre-R13 static-model path. |
