...
`audio.yml` (ffmpeg encode → object storage), `asr.yml` (faster-whisper transcription), and the
separate WhisperX known-text alignment lane. `audio.yml`
...
independent from native-tool provisioning. Local faster-whisper/WhisperX inference runs in one
...
only), the known-text lane installs `asr-align` (WhisperX), and diagnostics install `asr-bench`;
