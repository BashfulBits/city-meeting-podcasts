...
3. Set Worker secrets with `wrangler secret put` (never commit them):
   `FORMSPARK_WEBHOOK_SECRET`, `GITHUB_APP_ID`,
   `GITHUB_APP_INSTALLATION_ID`, `GITHUB_APP_PRIVATE_KEY`, `DISCORD_WEBHOOK_URL`,
   `RESEND_API_KEY`, `MAIL_FROM`, and optionally `MAIL_REPLY_TO`.
4. Create a GitHub App with Issues read/write permission, install it only on this repository, and
...
5. Generate a URL-safe random `FORMSPARK_WEBHOOK_SECRET` and save it with `wrangler secret put`.
...
6. In Formspark's form settings, choose **Turnstile** under Spam Protection and add the
...
7. Configure the Resend sender/reply address at `citymeetings.fyi`, including the required SPF,
