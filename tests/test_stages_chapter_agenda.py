...
def test_agenda_stage_failed_batch_job_does_not_consume_dispatch_quota(tmp_path: Path):
    ...
    city = _make_city()
    storage = LocalStorage(root=tmp_path / "s", url_prefix="https://cdn")
    ep = _make_episode("ep-fail-dispatch")
    _put_storage_bytes(storage, ep.links["agenda_text_artifact_key"], b"Agenda")
    ...
    assert ctx.chapter_agenda_dispatches_count == 0
    assert ctx.chapter_agenda_dispatches_reserved == 0
    assert not ctx.chapter_agenda_dispatch_exhausted.is_set()
