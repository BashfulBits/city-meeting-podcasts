...
def _rec(uid, *, media_kind="hls", hosted_url=None, spec_hash=None, error=None,
         duration=3600, bytes_val=None, published="2026-05-01T00:00:00+00:00"):
    return {
        "uid": uid,
        "title": f"Meeting {uid}",
        "published": published,
...
def test_build_status_episode_taxonomy(tmp_path):
    ...
    records = {
        ...
        "r2": _rec("r2", hosted_url="http://cdn/r2.m4a", spec_hash="deadbeef000a",
                   bytes_val=10_000_000),       # stale (different spec)
...
def test_build_status_audio_bytes_exact_flag(tmp_path):
    ...
    records = {
        ...
        "b": _rec("b", hosted_url="http://cdn/b.m4a", spec_hash="legacy", bytes_val=None),  # missing
    }
    save_records(tmp_path, source_key(city), records)
...
def test_build_status_health_histogram(tmp_path):
    ...
    def _feed(slug, author, feed_url):
        return City(
            slug=slug, provider="granicus",
            source={"feed_url": feed_url},
            podcast_title=slug, podcast_author=author,
            podcast_email="", podcast_description="d",
        )
    ...
    save_records(tmp_path, source_key(ok_city),   {"e1": _rec("e1", hosted_url="http://cdn/e1.m4a", spec_hash="legacy")})
...
def test_audio_bytes_round_trip():
    ...
    ep = Episode(
        guid="g", title="t", published=datetime.now(UTC),
...
def test_city_row_provider_and_latest(tmp_path):
    ...
    save_records(tmp_path, source_key(a1), {"e1": _rec("e1", media_kind="direct", published="2026-03-01T00:00:00+00:00")})
    save_records(tmp_path, source_key(a2), {"e2": _rec("e2", media_kind="direct", published="2026-05-15T00:00:00+00:00")})
    save_records(tmp_path, source_key(b1), {"e3": _rec("e3", media_kind="direct", published="2026-01-20T00:00:00+00:00")})
...
def test_github_run_url_in_run_history(tmp_path):
    ...
    entry = {
        ...
        "built": 5, "skipped": 82, "errors": 0,
        "materialized": 8, "materialize_encoded": 7,
        "materialize_seconds": 698.0,
        "stages": {"audio": {"ran": 8, "encoded": 7, "credited": 1,
                              "reused": 41, "backlog": 0, "seconds": 698.0,
                              "bytes": 12_900_000, "errors": 0}},
        "github_run_id": "99999",
...
def test_audio_bytes_set_on_encode(tmp_path):
    ...
    from citypods.models import Episode, City
    from citypods.media import materialize_audio
    from datetime import datetime, UTC

    city = City(
        slug="bytes-test", provider="civicplus",
        source={"feed_url": "http://x.com/f"},
        podcast_title="t", podcast_author="a",
        podcast_email="", podcast_description="d",
    )
    ep = Episode(
        guid="g", title="t",
    ...
    class FakeStorage:
        ...
        def public_url(self, key): return f"https://cdn/{key}"
